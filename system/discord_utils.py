...
class Discord:
    ...
    async def fetch(self) -> None:
        self.guild = await self.client.fetch_guild(self.config["discord"]["guild_id"])
        self.premium_role = await self.__fetch_role__()
        self.admin_channel = await self.client.fetch_channel(self.config["discord"]["admin_channel"])
    ...
    async def __fetch_role__(self) -> Optional[discord.Role]:
        ...
        for role in roles:
            if role.id == self.config["discord"]["premium_role"]:
                return role
        return None
